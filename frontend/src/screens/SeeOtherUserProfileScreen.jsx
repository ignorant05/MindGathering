...
import { useGetUserInfoQuery } from "../redux/slices/authApiSlice"
...
const SeeOtherUserProfileScreen = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/v1/users/count/users/${userId}/blogs`).then(res => res.json()).then((data) => { setBlogsCount(data) }).catch((err) => { console.error(err) })
  }, [blogsCount, userId])

  useEffect(() => {
    fetch(`http://localhost:3000/api/v1/users/count/users/${userId}/comments`).then(res => res.json()).then((data) => { setCommentsCount(data) }).catch((err) => { console.error(err) })
              ...
              <div className="flex justify-center my-20 px-6 text-center  h-full rounded-lg lg:mt-0 xl:px-10">
                <div className="w-96 px-6 py-6 text-center rounded-lg lg:mt-0 xl:px-10">
                  <div className="space-y-4 xl:space-y-6">
                    <img className="mx-auto rounded-full h-36 w-36" src={user.ProfilePicture} alt="author avatar" />
                      ...
                      <div className="flex justify-center items-center flex-col space-y-3 text-lg font-medium leading-6">
                        ...
                        <div className="grid grid-cols-2 grids rows 3 gap-14">
                          <div><span className="font-bold text-lg col-span-1 row-span-3">Comments</span>
                            <div className="font-medium text-lg col-span-1 row-span-1">{blogsCount}</div>
                            <Link></Link>
                          </div>
                          <div><span className="font-bold text-lg col-span-1 row-span-1">Blogs</span>
                            <div className="font-medium text-lg col-span-1 row-span-1">{commentsCount}</div>
                            <Link></Link>
                          </div>
                        </div>
                      </div>
