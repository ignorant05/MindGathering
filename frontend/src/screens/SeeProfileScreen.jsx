...
const SeeProfileScreen = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/api/v1/users/count/my/blogs").then(res => res.json()).then((data) => { setBlogsCount(data) }).catch((err) => { console.error(err) })
  }, [blogsCount])

  useEffect(() => {
    fetch("http://localhost:3000/api/v1/users/count/my/comments").then(res => res.json()).then((data) => { setCommentsCount(data) }).catch((err) => { console.error(err) })
  }, [commentsCount])
              ...
              <div className="flex justify-center my-20 px-6 text-center  h-full rounded-lg lg:mt-0 xl:px-10">
                <div className="w-96 px-6 py-6 text-center rounded-lg lg:mt-0 xl:px-10">
                  <div className="space-y-4 xl:space-y-6">
                    <img className="mx-auto rounded-full h-36 w-36" src="../../public/assets/default_picture.png" alt="author avatar" />
